...
У нас есть 3 таблицы `пользователи` `книги` `магазины`
Таблица `пользователи` состоит из `id_user`, `id_book`, `username`, `password`.
Таблица `книги` состоит из `id_book`, `name`, `author`, `year`.
Таблица `магазин` состоит из `id_shop`,`id_book`, `quantity`.
...
Таблицу `пользователи` мы можно горизонтально шардировать по серверам магазинов основываясь на географическом положении активности пользователя - `id_user`.
Таблице `книги` горизонтальный шардинг не требуется, таблица может целиком храниться на центральном сервере, для обеспечения единого управления каталогом книг.
Таблица `магазин`можно горизонтально шардировать по `id_shop`чтобы каждый сервер магазина содержал информацию только о наличии книг в конкретном магазине.
...
Таблице `книги` целиком будет храниться на центральном сервере для обеспечения единого управления каталогом книг, магазинам нет необходимости в дополнительной информации о книге.
Таблица `магазин` Целиком хранится на серверах магазинов. (для учёта наличия книг непосредственно в магазинах).
